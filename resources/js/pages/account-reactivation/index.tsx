...
export default function ReactivationRequests({ requests, highlight }: { requests: ReactivationRequest[]; highlight?: number | null }) {
    ...
    const review = (request: ReactivationRequest, action: 'approve' | 'reject') => {
        const send = () => router.post(`/account-reactivation-requests/${request.id}/${action}`, { admin_remarks: remarks[request.id] ?? '' }, { preserveScroll: true });

        if (action === 'approve') {
            void confirmDialog({
                title: `Reactivate ${request.user.name}'s account?`,
                description: "This will restore the resident's ability to log in.",
        ...
        if (!remarks[request.id]?.trim()) {
            void confirmDialog({ title: 'Add your remarks first', description: 'Admin remarks are required when rejecting a reactivation request.', notice: true });
